...
class Player extends React.Component<PlayerProps, TPlayerState> {
  ...
  getReorderedTracks(values: { moved: number; target: number }) {
    const movedTrackIndex = +this.state.tracks.findIndex((track: TrackType) => track.id === values.moved);
    const targetTrackIndex = +this.state.tracks.findIndex((track: TrackType) => track.id === values.target);
  ...
  skip(direction: string) {
    ...
    const trackIndex = tracks.findIndex((item: TrackType) => item.id === track.source.id);
    const index =
      direction === "next"
            ...
            <div className='grid'>
              <div className='grid__col-12 mb-0'>
                <PlayerSC data-testid='player' theme={theme} className='player'>
                  ...
                  <Progress theme={this.props.theme} percent={track.percent} track={track} />
