...
const RangeSlider: React.FunctionComponent<SliderProps> = React.memo((props: SliderProps) => {
  ...
  const setValue = (value) => {
    const thumbValue = value > 0 || value < 100
      ? 100 - value
      : value >= 100 ? 0
        : 100;
    setPercent(value);
  ...
  const moveThumb = (view) => {
    ...
    const views = {
      "round-horizontal": (params: {
        ...
        let shiftX = params.clientX - params.currentTarget.getBoundingClientRect().left;
        let newLeft = params.clientX - shiftX - left;
        let rightEdge = width - params.offsetWidth;
        newLeft = newLeft < 0
          ? 0
          : newLeft > rightEdge
            ? rightEdge
            : params.clientX - shiftX - left;
  ...
  const onMouseDown = React.useCallback((event: React.MouseEvent<HTMLElement>) => {
    ...
    let shiftY = clientY - currentTarget.getBoundingClientRect().top;
  ...
  const onClick = React.useCallback((event: React.MouseEvent<HTMLElement>): void => {
  ...
  const onWheel = React.useCallback((event) => {
    let per = percent < 100 && event.deltaY < 0
  ...
  const processLineFill = props.orientation === "horizontal"
    ? { width: `${percent}%` }
    : { height: `${percent}%` };
  return (
    <RangeSliderSC
