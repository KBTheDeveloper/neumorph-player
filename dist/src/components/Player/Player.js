...
class Player extends React.Component {
        ...
        const howl = new Howl({
            ...
            onend: function () {
                if (self.state.settings.repeat === "once") {
                    ...
                    self.play(true, shuffle(self.state.tracks)[0].id);
                else
                    self.skip("next");
            },
            onseek: function () {
    ...
    render() {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(PlayerWrapperSC, { position: position, className: 'playerWrapper' },
                React.createElement("div", { className: 'container' },
                    React.createElement("div", { className: 'grid' },
                        React.createElement("div", { className: 'grid__col-12 mb-0' },
                            React.createElement(PlayerSC, { "data-testid": 'player', theme: theme, className: 'player' },
                                React.createElement(TrackInfo, { title: track.source.title, cover: track.source.cover, artist: track.source.artist, track: track.source.howl, duration: track.duration }),
                                React.createElement("div", { className: 'controlsOuter px-6 px-xs-4' },
                                    React.createElement("div", { className: 'controlsInner d-flex align-items--center' },
                                        React.createElement(IconWrapper, { title: 'Playlist', role: 'button', tabIndex: '0', noAlign: true, styles: defaultButtonStyle, classes: 'mr-5', iconName: 'playlist', onKeyPress: this.togglePlaylist, onClick: this.togglePlaylist }),
                                        React.createElement(Controls, { theme: theme, loading: track.loading, currentTrackIndex: track.index, items: tracks, onSkip: this.skip, onPlay: this.onTrackChange, playback: playback, onPause: this.onTrackChange, 
                                            // videoPlayer={
                                            //   <VideoPlayer
                                        ...
                                        React.createElement(Volume, { level: settings.volume, theme: theme, muted: settings.muted, slider: {
                                                value: settings.volume,
                                                start: 50,
                                            } }))),
                                React.createElement(Progress, { theme: this.props.theme, percent: track.percent, track: track }),
                                React.createElement(Suspense, { fallback: 'loading' },
                                    React.createElement(Playlist, { show: playlist, theme: theme, playback: this.onTrackChange, onReorder: this.getReorderedTracks.bind(this), onPlay: this.onTrackChange, onPause: this.onTrackChange, currentTrack: track, items: tracks, onClose: this.onClosePlaylist })))))))));
    }
}
